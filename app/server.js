...
// API
const apiKeySecret = process.env.API_KEY_SECRET || 'videocall_default_secret';
...
app.post([`${apiBasePath}/join`], (req, res) => {
    ...
    if (!api.isAuthorized()) {
        log.debug('VideoCall get join - Unauthorized', {
            header: req.headers,
            body: req.body,
        });
    ...
    log.debug('VideoCall get join - Authorized', {
        header: req.headers,
        body: req.body,
        join: joinURL,
    });
